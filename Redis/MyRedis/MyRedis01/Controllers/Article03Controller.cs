...
namespace MyRedis01.Controllers
{
    ...
    public class Article03Controller : ControllerBase
    {
        ...
        public async Task<ActionResult> Create(CreateArticleDto createArticle)
        {
            ...
            await redisDatabase.ListAddToLeftAsync(ArticleConstant.ArticleList, article.Id);
        ...
        public async Task<ActionResult> Delete(long id)
        {
            ...
            await redisDatabase.Database.ListRemoveAsync(ArticleConstant.ArticleList,id);
        ...
        public async Task<ActionResult> GetArticles([FromQuery] ArticleParameter parameter)
        {
            ...
            var ids = await redisDatabase.Database.ListRangeAsync(ArticleConstant.ArticleList, starIndex, endIndex);
            var result = new List<Dictionary<string, object>>();
            foreach (var id in ids)
            {
                var article = new Dictionary<string, object>();
                var title = await redisDatabase.HashGetAsync<object>(ArticleConstant.ArticleData + id, "Title");
                article.Add("id", id);
